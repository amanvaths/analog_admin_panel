...
class SideMenuContent extends React.Component {
  ...
  render() {
    ...
    // const permissionEM = ['order','users','dashboard'];
    const {permissionEM, permissionSub} = this.state
    // const permissionEM = ['order','users','dashboard'];;
    console.log("permissionEM: ",permissionEM, permissionSub)
    const menuItems = navigationConfig.map(item => {
      const CustomAnchorTag = item.type === "external-link" ? `a` : Link
      // checks if item has groupheader
      let parent      = item.parentof ? item.parentof : ''; 
      let parentId      = item.parentof ? item.id : ''; 
      let npermissionEM = isArray(permissionEM) ? permissionEM.includes(parent) : false ;
      let npermissionSUB = isArray(permissionEM) ? permissionEM.includes(parentId) : false ;
      let isEmpPermission = (this.props.currentUser == 'admin') ? true : npermissionEM;
      let ispermissionSUB = (this.props.currentUser == 'admin') ? true : npermissionSUB;
      
      if ((item.type === "groupHeader" && isEmpPermission) ) {
        return (
          <li
      ...
      // console.log("permissionEM: ", permissionEM)
      
      let renderItem =  '';
      if(isEmpPermission || ispermissionSUB){
        renderItem =  (
          <li
