...
class UsersList extends React.Component {
  ...
  async componentDidMount() {
    ...
    getAPICall('get-all-order?action=sell&status=0&admin_user_id='+this.state.currentUserId + params)
    .then(response => {
    ...
    if (this.state.gridApi !== prevState.gridApi) {
      const dataSource = {
        getRows: (rowData) => {
          ...
          let params = '?action=sell&status=0&admin_user_id='+this.state.currentUserId + "&per_page=" + this.state.pageSize + "&page=" + page;
          ...
          // console.log(params);
          getAPICall('get-all-order' + params).then((res) => {
  ...
  //     }
  //   });
  //   getAPICall('get-all-order?action=sell&status=0&admin_user_id='+this.state.currentUserId)
  //   .then(response => {
  //     let rowData = response.data.result;
  //     let total_price = response.data.total_price;
  //     let total_volume = response.data.total_volume;
  ...
  render() {
    const { rowData, rowDataSell,total_price,cPaired, cmcDATA , total_volume, total_count, columnDefs, defaultColDef, pageSize } = this.state;
    let getPrice =  {}; 
    ...
    const PCard = (total_price != null ) && (cmcDATA != null) && cmcDATA.map(item => {
      let totl = total_price[item.symbol] ? Number(total_price[item.symbol]).toFixed(2) : 0
    ...
    const VCard = (total_volume != null ) && (cmcDATA != null) && cmcDATA.map(item => {
    ...
    const TCard = (total_count != null ) && (cmcDATA != null) && cmcDATA.map(item => {
            ...
            <CardBody>
              <div className="ag-theme-material ag-grid-table">
                <div className="ag-grid-actions d-flex justify-content-between flex-wrap mb-1">
                  ...
                  <div className="">
                    <div className="h2 float-left">Total Open Sell : {this.state.count !== null ? this.state.count : 0 }</div>
                  </div>
